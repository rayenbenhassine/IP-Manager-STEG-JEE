 ...
 * @author rayen
 */
public class RechercherAdministrateur extends HttpServlet
{

	public RechercherAdministrateur()
	{
		super();
	}
	@Override
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		{
			ResultSet result = ConnexionBD.executeSelectQuery("select * from user where matricule = " + request.getParameter("matricule") + " and admin = 1");
			try {
				if(result.first())
				{
		...
		this.getServletContext().getRequestDispatcher("/listeAdministrateurs.jsp").forward(request, response);
	}
